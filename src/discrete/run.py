...
def run(_run, _config, _log):
    ...
    if args.use_tensorboard:
        tb_logs_direc = os.path.join(dirname(dirname(dirname(dirname(abspath(__file__))))), "results", "tb_logs")
...
def run_sequential(args, logger):
    ...
    mac = mac_REGISTRY[args.mac](off_buffer.scheme, groups, args)

    # Give runner the scheme
    runner.setup(scheme=scheme, groups=groups, preprocess=preprocess, mac=mac)

    # Learner
    learner = le_REGISTRY[args.learner](mac, off_buffer.scheme, logger, args)
    ...
    while runner.t_env <= args.t_max:
        ...
        if off_buffer.can_sample(args.off_batch_size):
            # off samples
            episode_sample = off_buffer.sample(args.off_batch_size)
